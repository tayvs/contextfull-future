...
//
//  class ThreadLocalMapWrapper (threadLocalMap: ThreadLocal.ThreadLocalMap) {
...
//    }
//
//    def value: ThreadLocal.ThreadLocalMap = threadLocalMap
//
//  }
//
...
//  def copyThreadLocalMap(tlm: ThreadLocal.ThreadLocalMap): ThreadLocal.ThreadLocalMap = {
//    if (tlm == null) tlm
//    else {
//      val entityArr = getTableThreadMapMethod.get(tlm).asInstanceOf[Array[ThreadLocal.ThreadLocalMap.Entry]]
...
//        val newInstance: ThreadLocalMapWrapper = new ThreadLocalMapWrapper(new ThreadLocal.ThreadLocalMap(head.get(), head.value))
////        tail.map(new EntryWrapper(_)).foreach(entry => setThreadLocalMapMethod.invoke(newInstance, entry.key, entry.value))
//        tail.foreach(entry => newInstance.set(entry.get(), entry.value))
//        newInstance.value
//      } // else null
...
//    threadLocalsField.set(to, copyThreadLocalMap(threadLocalsField.get(from).asInstanceOf[ThreadLocal.ThreadLocalMap]))
//    val end = System.nanoTime()
//
//    println(s"threadLocalMap reassigning takes " + (end - start))
//    //    println(f.get(t).asInstanceOf[ThreadLocals])
//  }
//
//  def getThreadLocalMap(from: Thread): ThreadLocal.ThreadLocalMap = copyThreadLocalMap(threadLocalsField.get(from).asInstanceOf[ThreadLocal.ThreadLocalMap])
//
//  def assignThreadLocalMap(to: Thread, tlm: ThreadLocal.ThreadLocalMap): Unit = threadLocalsField.set(to, tlm)
//
